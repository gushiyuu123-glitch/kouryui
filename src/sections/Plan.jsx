// src/sections/Plan.jsx
import { useEffect, useRef } from "react";
import styles from "./Plan.module.css";
import { RevealText, RevealImage } from "../components/Reveal";

const PLANS = [
  {
    no: "01",
    en: "BENI WALK",
    jp: "紅歩きプラン",
    title: "昼は、光がいちばん素直。",
    poem: [
      "昼の石畳で、衣を着て歩く。",
      "光がいちばん素直に映る。",
      "初めてでも決めやすい。",
    ],
    badge: "標準｜散策向け",
    price: "¥9,800~",
    duration: "所要：2時間前後（準備含む）",
    returnBy: "返却：〜19:00",
    img: "/plan-011.png",
    alt: "昼の石畳の道で、紅の衣を着て歩く。",
    ratio: "24/9",
    start: "top 82%",
  },
  {
    no: "02",
    en: "KOKUSAI LINE",
    jp: "国際通りプラン",
    title: "夕暮れは、色が深くなる。",
    poem: [
      "日が傾くと、色が深くなる。",
      "影が出て、写真が締まる。",
      "選ばれている時間帯。",
    ],
    badge: "夕暮れ枠｜選ばれている時間帯",
    price: "¥11,800~",
    duration: "所要：2時間前後（準備含む）",
    returnBy: "返却：〜19:00",
    img: "/plan-022.png",
    alt: "夕方の街で、紅の衣が光を受ける。",
    ratio: "21/10",
    start: "top 80%",
  },
  {
    no: "03",
    en: "NIGHT BENI",
    jp: "夜紅プラン",
    title: "灯りで、輪郭が出る。",
    poem: [
      "国際通りの灯りが輪郭を作る。",
      "昼とは別の顔が出る。",
      "夜が好きな人へ。",
    ],
    badge: "夜間対応｜灯りの時間",
    price: "¥13,800~",
    duration: "所要：2時間前後（準備含む）",
    returnBy: "返却：〜21:00",
    img: "/plan-033.png",
    alt: "夜の灯りの中で、紅の衣が輪郭を立てる。",
    ratio: "22/10",
    start: "top 78%",
  },
];

export default function Plan() {
  const wrapRef = useRef(null);

  useEffect(() => {
    const el = wrapRef.current;
    if (!el) return;

    const reduce =
      window.matchMedia?.("(prefers-reduced-motion: reduce)")?.matches;

    const setWrapIn = () => el.classList.add(styles.in);

    // overlay/stamp を “画像と同じ出番” で出す
    const stageWraps = Array.from(el.querySelectorAll(`.${styles.stageWrap}`));
    const stageObservers = [];

    const armStageWrap = (node) => {
      const raw = node.dataset.ovStart || "top 82%";
      const m = raw.match(/top\s+(\d+)%/);
      const pct = m ? Number(m[1]) : 82;
      const cut = Math.max(0, Math.min(60, 100 - pct));
      const rootMargin = `0px 0px -${cut}% 0px`;

      const io = new IntersectionObserver(
        ([entry]) => {
          if (!entry.isIntersecting) return;
          node.dataset.ov = "1";
          io.disconnect();
        },
        { threshold: 0, rootMargin }
      );

      io.observe(node);
      stageObservers.push(io);
    };

    if (reduce) {
      setWrapIn();
      stageWraps.forEach((n) => (n.dataset.ov = "1"));
      return;
    }

    const io = new IntersectionObserver(
      ([entry]) => {
        if (!entry.isIntersecting) return;
        setWrapIn();
        io.disconnect();
      },
      { threshold: 0.22, rootMargin: "-10% 0px -10% 0px" }
    );
    io.observe(el);

    stageWraps.forEach(armStageWrap);

    return () => {
      io.disconnect();
      stageObservers.forEach((o) => o.disconnect());
    };
  }, []);

  return (
    <section id="plan" className={styles.section} aria-label="プラン">
      <div ref={wrapRef} className={styles.wrap}>
        {/* 右：縦コピー（絶対に触らない） */}
        <aside className={styles.vertical} aria-hidden="true">
          <div className={styles.verticalInner}>
            <span className={styles.v1}>迷うなら、</span>
            <span className={styles.vBeni}>紅</span>
            <span className={styles.v2}>で決める。</span>
          </div>
        </aside>

        <div className={styles.stack}>
          {/* ✅ 色×プランの喧嘩を止める1行 */}
          <RevealText as="p" className={styles.desc} d="0.04s">
            色はプランに関係なく選べます。
          </RevealText>

          <ul className={styles.list}>
            {PLANS.map((p, i) => {
              const base = 0.02 + i * 0.10;

              return (
                <li key={p.no} className={styles.item}>
                  {/* 左：詩（具体）だけ */}
                  <div className={styles.meta}>
                    <RevealText
                      as="p"
                      className={styles.poem}
                      d={`${base + 0.10}s`}
                    >
                      {p.poem.map((line, idx) => (
                        <span key={idx} className={styles.poemLine}>
                          {line}
                        </span>
                      ))}
                    </RevealText>

                    {/* 読み上げ保険（色はここで言わない） */}
                    <span className={styles.srOnly}>
                      {p.no} {p.en} {p.jp}。{p.badge}。料金 {p.price}。{p.duration}
                      {p.returnBy ? `、${p.returnBy}` : ""}
                    </span>
                  </div>

                  {/* 右：舞台（画像 + overlay） */}
                  <div
                    className={styles.stageWrap}
                    data-ov-start={p.start}
                    style={{ "--ovDelay": `${base + 0.02}s` }}
                  >
                    <RevealImage
                      className={styles.stage}
                      imgClassName={styles.img}
                      src={p.img}
                      alt={p.alt}
                      ratio={p.ratio}
                      d={`${base + 0.02}s`}
                      mode="io"
                      start={p.start}
                    />

                    <div className={styles.overlayTop} aria-hidden="true">
                      <div className={styles.overlayNo}>{p.no}</div>
                      <div className={styles.overlayEn}>{p.en}</div>
                      <div className={styles.overlayJp}>{p.jp}</div>

                      <div className={styles.overlayBadge}>
                        <span className={styles.badgeText}>{p.badge}</span>
                      </div>
                    </div>

                    <div className={styles.stamp} aria-hidden="true">
                      <div className={styles.stampPrice}>{p.price}</div>

                      <div className={styles.stampTime}>
                        <span className={styles.duration}>{p.duration}</span>
                        {p.returnBy ? (
                          <span className={styles.returnBy}>{p.returnBy}</span>
                        ) : null}
                      </div>
                    </div>
                  </div>
                </li>
              );
            })}
          </ul>

          <div className={styles.bottomMark} aria-hidden="true">
            <div className={styles.smallCaps}>KIMONO RENTAL OKINAWA</div>
            <div className={styles.beniCaps}>BENI COLLECTION</div>
          </div>
        </div>
      </div>
    </section>
  );
}